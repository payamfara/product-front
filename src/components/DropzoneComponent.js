...
const DropzoneComponent = ({ uploadedFiles = [], uploadUrl }) => {
  ...
  const isDuplicateFile = (fileUrl) => {
    return files.some((f) => f.url === fileUrl);
  };
  useEffect(() => {
  ...
  const handleGallerySubmit = (selectedUrls) => {
    console.log("Uploaded Urls:", selectedUrls);
